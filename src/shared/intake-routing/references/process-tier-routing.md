...
- **低难度**：不触发档位建议入口，通常由人工确认是否采用 L0。
- **中难度**：触发建议，默认提示 L1；材料命中状态、异常、校验、权限、敏感数据、多角色、多系统或迁移等信号时提示 L2。
- **高难度**：触发建议，默认提示 L2。

上述结果只写入 `difficulty_level` 与 `tier_recommendation` 作为入口记录，不能创建目录、改变 work item 或修改 `process_tier`。最终档位必须由人工选择，并通过 `pipeline.py init --process-tier L0|L1|L2` 写入 `00-input/intake-decision.md`；`gate`、`review`、`reflow` 只读取该持久化档位。建议与资格矩阵冲突时，以人工选择后的资格矩阵复核为准，命中硬升级条件必须升级。
