...
- **使用方式**：复制到 function-description 对应章节表格时——① 将 `FUN-XXX` 改为目标产物中的实际 FUN 编号（如 FUN-008）；② BR/AC/EX ID 改为目标产物内单调递增编号，避免与既有行冲突；③ 「来源」列保留本片段路径，并可追加原始 SRC 追溯；④ 字符上限（2-16 / 1-8 / 200）与「可中杠下划线」等细则按目标案例 BRD 校准；⑤ 若目标案例无昵称记忆需求，删除 BR-XXX-4/AC-XXX-3 并注明。
