...
写入独立产物 `state-machine.md` 的 §状态变化 部分。提交前逐项自检。
...
- [ ] `python3 scripts/validate_artifact.py <state-machine.md> --json` 返回 `"ok": true`
...
- [ ] 对每个存在状态的实体（P0 功能的 `FEA-XXX`）逐一列出全部合法状态，无遗漏、无杜撰状态
...
- [ ] 转移表中的每条 STATE-XXX 均可反追溯到上游 `FEA-XXX` 与来源文档，正向可追踪、反向可溯源
- [ ] 状态命名统一（中英文混用已消除、同义不同名状态已合并），读者不会把「待审核 / 审核中」当成两个状态
- [ ] 终态与取消/终止语义一致：不会出现「取消后还能回到进行中」的语义矛盾
- [ ] 与独立产物 `state-machine.md`「事实与决定」「来源追溯」章节的引用一致，无孤儿引用、无悬空链接
...
- [ ] 本 work_item 产出独立产物 `state-machine.md`（§状态变化），且未改动其他产物
