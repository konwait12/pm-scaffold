...
每次提交 validation-rules.md 产物前自审。先跑确定性校验器，再逐条过本清单。
...
- [ ] `python3 scripts/validate_artifact.py <validation-rules.md> --json` 返回 `"ok": true`
...
- [ ] 该 `FEA-XXX` 下所有需要用户输入的字段都被枚举，无遗漏必填/可选输入项
...
- [ ] 校验规则与 `FEA-XXX` 功能目的一致：该功能确有此输入与校验需求，而非凭空发明字段
...
- [ ] 每个 `VL-XXX` 都标注来源：`FEA-XXX` → `ST-XXX`/`BR-XXX`，能一路追溯到已确认事实或决策
...
- [ ] 本 work_item 产出已写入独立产物 `validation-rules.md`，且未改动其他 work_item 产物
