...
def validate(path: Path) -> dict[str, object]:
    ...
    # Rule statements: BR rows should have non-placeholder content
    # Look for table rows with BR-XXX but no actual rule text
    br_rows = [
        line.strip() for line in text.splitlines()
        if "BR-" in line and line.strip().startswith("|")
    ]
    empty_br_rows = [
        line for line in br_rows
        if len(line.replace("|", "").replace("-", "").replace(" ", "").strip()) < 5
    ]
    if empty_br_rows:
        errors.append(
            f"Found {len(empty_br_rows)} BR-XXX rows with missing rule statements. "
            "Each BR-XXX must have a complete rule statement, not just the identifier."
        )
