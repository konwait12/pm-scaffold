...
| 列 | 要求 |
|---|---|
| ID | `EX-XXX`，全局唯一、稳定且不可复用；允许历史空洞，详见 `src/framework/id-contract.md`。填写 `FUN/FEA/BR/STATE/SRC` 等追溯锚点，跨功能异常写 `GLOBAL`，不与 BR-/VL-/ST-/AC-/IX- 混淆 |
...
- 每条 `EX-XXX` 必须有可解释追溯锚点；流程异常优先关联 `FUN/FEA`，实体/合规/平台异常可关联 `BR/STATE/SRC` 或显式 `GLOBAL`。
- 与已确认的 `BR` 异常分支 / `ST` 异常路径建立关联（关联 BR / 关联状态迁移），下游 `AC` 才能覆盖验证。
- ID 不重复且不复用；一条 EX 只描述一种失败，多个失败拆成多行。
