...
- [ ] 每条 IX 可追溯至 page-design 的对应页面与 ux-flow 的对应流程
- [ ] IX 覆盖范围与 ux-flow 中出现的页面一致，无孤儿规则、无缺失页面
...
每条 IX 按单条需求的九特征抽查：可验证（响应可写判据）、无歧义（触发唯一）、单一（一条一个交互）、必要（删掉丢反馈）、一致（适用页面存在于 §4）。"响应模糊"（如"给出合理提示"）为阻断项：要么改为具体动作/状态，要么 `needs_user_input`。
