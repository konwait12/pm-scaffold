# Source Handling · ux-flow

本子 Skill 消费的是**已确认的上游产物**而非原始材料。因此"来源"指：上游 `user-journey-and-stories`、父产物 `product-ux` §2 功能清单、以及更早的 `SRC-*` 原始来源。
...
1. 先逐字摘取上游对流程的陈述（入口、步骤、分支、出口），再结构化为图。
2. 步骤命名与上游原文一致；图内缩写必须在本图或 §4 首次出现处解释。
3. 为补全图而增加的步骤：分离"故事里写了"（FACT）与"我为了图完整补的"（AI_INFERENCE）。
4. 排除的上游流程内容要写理由（重复 / 出范围 / 已被更新），不静默丢弃。
5. 上游未提的空白（如空态、超时）**不视为确认不存在**——标 `UNKNOWN` 交人工。

## Authority And Conflicts

当上游范围与图面表达冲突时：

- 保留双方：保留上游原文表述与图面的选择，都标 ID。
- 解释影响：不同路径会怎样改变下游页面与规则。
- 标 `CONFLICT-XXX` 并停止：若该冲突改变 P0 主路径或关键分支，交给产品负责人裁决。
- **绝不静默选择更方便的路径。**
...
- 可自行调研：平台通用 UX 惯例、常见流程模式（如登录回跳、空态引导）——这些是可发现的事实，标记来源。
- 不可自行确认：本期范围的入口取舍、P0 主路径、跨系统交接职责——交人工。
- 外部调研结论一律先标 `AI_INFERENCE`，需人工确认后才能当决策依据。

## Mixed Media

- 从截图/旧原型反推流程时，先转写可见的页面与跳转关系，标注转写不确定性。
