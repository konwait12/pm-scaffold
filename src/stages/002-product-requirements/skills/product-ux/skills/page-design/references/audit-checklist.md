# Audit Checklist · page-design

对 `product-ux` 制品 §4「页面与原型」进行提交前自查。从下游逐项检查，任何一项不通过都先修正再提交。

## 1. Structural Gate（结构闸口）

- [ ] 父制品中存在 `## 4. 页面与原型` 段落（或同级规范化的 `页面与原型` 标题）。
- [ ] 段落中的每一行页面条目都归属某个 `FEA-\d+` 功能，所属功能列不为空且可追溯到 §2 功能清单。
- [ ] 页面条目来自 `ux-flow` 输出的 UX 流程，没有凭空新增的孤儿页面。
- [ ] 运行 `scripts/validate_artifact.py` 能通过，或至少能解释每个 ERROR 与 WARNING 的处置。

## 2. Content Gate（内容闸口）

- [ ] 每个页面/步骤都占一行，七列齐全：页面/步骤名、所属功能、入口、前置条件、主要内容、操作、下一状态。
- [ ] 每个页面至少有 1 个可执行操作，没有"只能看不能做"的悬空页面。
- [ ] 每个操作都定义了明确的下一状态（含"停留本页"或"退出流程"等显式结果），没有悬空箭头。
- [ ] 入口写明触发条件（用户行为、外部事件、跳转来源），前置条件写明进入该页面时必须已满足的状态或数据。
- [ ] 主要内容只描述"页面上有什么"（信息区块、列表、表单字段、确认信息），不描述视觉细节。
- [ ] UX 流程中出现的所有页面/步骤在此都有条目，数量与 `ux-flow` 对齐。
- [ ] P0 流程相关的页面覆盖正常、备选、异常、失败、超时、取消、恢复等已识别路径，不只画主路径。

## 3. Boundary Gate（边界闸口）

- [ ] 未描述视觉设计（颜色、字体、间距、组件样式、视觉层级）——那是原型/前端的事。
- [ ] 未写交互微细节（动效、悬停、滚动行为、弹窗出现方式）——那是 `interaction-rules`（IX）的事。
- [ ] 未写业务规则、校验规则、状态机细项、验收标准——那是 `function-description`（BR/VL/AC）的事。
- [ ] 未出现数据库表结构、API 设计、架构设计、权限策略细节。
- [ ] 未出现 `IX-`、`BR-`、`VL-`、`AC-` 前缀的规则编号（除引用外不在此生产）。

## 4. Semantic Gate（语义闸口）

- [ ] 每个页面条目可追溯回对应的 `FEA-\d+` 及其上游用户故事/旅程。
- [ ] 不确定的内容标注了知识状态标签（`FACT` / `DECISION` / `AI_INFERENCE` / `UNKNOWN`），而不是把推断写得像事实。
- [ ] 入口、前置条件、下一状态之间没有逻辑矛盾（例如下一状态引用了不存在或未定义的页面）。
- [ ] 对同一功能的多页面拆分有明确判断依据，不是简单按"屏幕"切页。
- [ ] 若删除了某个页面，能在 `ux-flow` 和上游故事中找到对应依据。

## 5. Quality Lenses

- 第一性原理：删掉页面上的某个信息区块/操作后，用户能否仍达成该步目标？哪些是"常见页面都有"但本功能不需要的？
- 系统思考：前置条件涉及的权限/数据是否依赖其他角色或外部系统？哪些已确认、哪些会随上游变化？
- 对抗性审查：至少为一个前置条件构造失败反例（不满足时用户会怎样？会被引导到哪），验证下一状态存在。
- 逆向验证：从每个操作的下一状态反推，本页内容/操作是否足够支撑到达？
- 最小充分：页面包含下游需要的七列信息，且不包含视觉、交互微细节与规则。

## 6. Requirement Quality Gate（29148 特化）

每个操作与下一状态按单条需求的九特征抽查：可验证（下一状态可写判据）、无歧义（操作意图唯一）、单一（一行一个操作）、必要（删掉会丢路径）、一致（下一状态存在于清单）。"操作后结果含糊"（如"更新状态"无去向）为阻断项：要么写明去向，要么 `needs_user_input`。

## 7. Human Gate（人工闸口）

- [ ] 没有遗留 `待确认` / `TBD` 占位符的列（若有，已归入"待确认问题"段落并在提交前向产品澄清）。
- [ ] 已与 `ux-flow`、§2 功能清单交叉核对过页面名称与 FEA 编号一致。
- [ ] 页面数量、操作、下一状态已由产品/业务确认，尤其是 P0 主路径。
- [ ] 本人（Page Design 子技能执行者）确认本段落可独立支撑 `product-ux` 的评审，无需 AI 现场解释。

## Audit Report Shape

```text
status_recommendation
passed_checks
failed_checks
repairs_applied
blocking_questions
nonblocking_unknowns
decisions_required
traceability_gaps        # 无 FEA/流程追溯的页面
downstream_risks         # 对 interaction-rules / function-description 的影响
```
