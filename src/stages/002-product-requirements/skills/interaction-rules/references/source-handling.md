...
本 work_item 消费的是**已确认的上游产物**而非原始材料。因此"来源"指：上游 `page-design.md` 页面清单（§页面设计）、`functional-flow.md` 功能流程（§主流程 / §分支流程 / §异常流程）、`feature-list.md` 功能清单，以及更早的 `SRC-*` 原始来源。
...
```text
rule_id            # IX-XXX，本 work_item 生成的规则 ID
applicable_page    # PG-XXX，来自 page-design.md §页面设计
...
2. 规则中的术语（页面名、操作名、状态名）与 page-design.md §页面设计、functional-flow.md 功能流程一致，不自造。
...
上游产物的版本关系：`page-design.md` / `functional-flow.md` 为 `confirmed` 才有权作为规则输入；`superseded` 版本的页面/流程不采用，除非确认的变更记录明确沿用。
...
每条规则的落位填入独立产物 `interaction-rules.md` 的 `## 来源追溯`：来源 ID、材料/位置、关键内容、本文落位或排除理由。规则与来源的对应关系要在 §来源追溯 中可查。
