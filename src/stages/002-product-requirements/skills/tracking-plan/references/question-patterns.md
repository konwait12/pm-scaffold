# Question Patterns · Tracking Plan
...
**When to use**: when an event's purpose is unclear or it is a candidate for "track everything" noise.
...
**When to use**: when `trigger_condition` is ambiguous or two engineers would fire the event differently.
...
**When to use**: when a property's meaning or domain is unclear and would pollute analytics.
...
**When to use**: when a property contains identifiers, fingerprints, or sensitive content.
...
**When to use**: when the metric needs timeliness that the upload timing cannot support.
...
**When to use**: when the same function exists on multiple platforms and event coverage differs.
...
**When to use**: when the metric type for an event is unclear or mislabeled.
...
**When to use**: when a P0 function has no must_track event, or a G-X cannot be verified.
