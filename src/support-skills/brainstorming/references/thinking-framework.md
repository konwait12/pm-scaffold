...
## 三路径分类（Three-path Classification，Preflight）

在开跑前先把需求**复杂度**判成三态（AI 初判、人工可覆盖，替代"全部一股脑 12 维深挖"）。判定路径在开跑时口头宣告，供人工纠正：

| 路径 | 判定 | 发散强度 | Clarify | 处置 |
|---|---|---|---|---|
| **轻量澄清**（轻） | 单条方向明确、范围小、单角色 | 短发散（聚焦 1-2 个相关维度）| 2-3 问即止 | 4 值照常 |
| **标准发散**（标） | 常规 L0/稀疏需求、无跨模块 | 现有完整 12 维 | 常规 ≤5 问 | 4 值照常 |
| **全景发散**（景） | 跨模块 / 多系统交付 / 多角色 | 全 12 维 + 分段呈现 | 常规 ≤5 问 | 4 值照常 + 可选 viz |

- **轻量**：避免为小需求做整套发散，降低无效候选噪音。
- **全景**：跨模块 / 多系统 / 多角色时需要**分段呈现**（按模块/系统/角色分批展示候选，避免一张表堆不下），并可选用 `references/visual-companion.md` 的可视化呈现（纯增强，不阻塞主流程）。
- 判定疑误 → 取较重的标准发散，保守优先。
...
## YAGNI 削减（YAGNI Trim，在聚类去重后、写回前）

发散会过度产出。在聚类去重后、写回前，对候选做一步**需求目标过滤**（AI 初判、人工拍板）：

| 提取条件 | 处置初判 | Reason 注明 |
|---|---|---|
| 对本需求目标**无贡献**（离题、镜花水月） | `exclude` | 对目标无贡献的原因 |
| **成本高收益低**（如重造既有系统能力、周期远超目标） | `defer` 优先 | 高成本 vs 低收益的对比 |
| 纯实现幻想、无业务诉求支撑 | `research` 或 `exclude` | 缺诉求来源 |

规则：

- AI **只给初判**，最终 `include/exclude/defer/research` 始终由 business_owner 人工拍板。
- 削减不删除候选：被划入 `exclude`/`defer` 的候选仍留在候选表里（处置表可见），只是更可能被人工排掉。
- 削减依据必须写进 Reason / Evidence，避免"想砍就砍"。
- 目的：让输入包更聚焦本需求目标，而不是把发散的所有产物都往里塞。
