...
**做**：分析市场空间、技术可行性、投入产出、风险评估，给出明确推荐：做 / 不做 / 有条件做（条件须具体可衡量）。

**不做**：替代 background-goal；替业务方做业务决策（只提供证据与推荐）；扩写成完整商业计划或技术架构。
...
使用 `src/templates/support/feasibility-report.md` 中的模板（相对项目根）：`## 市场空间` / `## 技术可行性` / `## 投入产出` / `## 风险评估` / `## 结论`.
