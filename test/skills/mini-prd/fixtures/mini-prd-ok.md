...
|---|---|---|---|---|---|
| 受影响角色与结果 | 客人看到与后端一致的报名截止日期 | FACT | SRC-003 §活动配置 | nova | 角色或入口变化 |
| 采用方案与被排除替代 | 修改展示字段；不改后端配置，避免扩大范围 | DECISION | mini-prd §1/§3 | nova | 需要改数据时 |
| 价值-成本-风险 | 消除日期误导；单字段变更成本低；无数据写入风险 | FACT | SRC-003 §配置 | nova | 后端契约变化 |
| 失败边界与回退 | 空日期显示待定；回退为恢复原字段 | FACT | mini-prd §5 | nova | 失败语义变化 |
| 可证伪条件/停止条件 | 若后端日期仍不可信，停止前端修补并升级接口问题 | DECISION | SRC-003 §配置 | nova | 数据源异常 |
