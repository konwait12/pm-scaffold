...
Spec Kit 等 spec-driven 工具是**研发向**的（spec → plan → tasks → implement），产品经理把需求写进 `spec.md` 后就不再被流程约束。本脚手架专攻**产品经理写 PRD 这一段**，用机器闭环把「AI 写需求」变成「AI 起草 + 人拍板 + 全程可追溯」的受控流程。

| 能力 | PM Scaffold | Spec Kit | 一般 PRD Skill |
|---|---|---|---|
| 目标交付 | 人工确认的中文 `prd.md` | 可执行的技术 spec | 一次性 PRD 文本 |
| 六态知识标注（FACT/DECISION/ASSUMPTION/AI_INFERENCE/UNKNOWN/CONFLICT） | ✅ | ❌ | ❌ |
| 不可绕过的人工闸门（SHA-256 绑定 reviewer） | ✅ | 流程习惯 | ❌ |
| 多源输入整合（邮件/纪要/PPT/图片） | ✅ | ❌ | ❌ |
| Constitution 治理层 | ✅ | ✅ | ❌ |
| 验收追溯（G→ST→FEA→FUN→AC/BR 双向） | ✅ | 部分 | ❌ |
| 变更级联失效（reflow） | ✅ | ❌ | ❌ |
| 面向人群 | 产品经理 | 研发/全栈 | 产品经理 |
